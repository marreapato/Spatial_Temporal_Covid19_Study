...
$$Im = \frac{n\sum_{i=1}^{n}\sum_{j=1}^{n}(x_{i}-\overline{x})(x_{j}-\overline{x})w_{ij}}{W\sum_{i=1}^{n}(x_{i}-\overline{x})^2}$$
 ...
 Onde $s^{2}_{i}$ é a variância da variável de estudo $x_{i}$:
  ...
  Os mapas de correlação local de Moran, os *Boxmaps* e as tabelas de moran global, foram feitas para as variáveis (número de recuperados, casos ou mortes, sendo algumas destas) que atingissem correlação global IM>0,50. (falar dos boxmaps) houve no entanto a preferência pelo número mensal do que o acumulado, para contornar o possível efeito da sazionalidade e capturar a dependência local apenas em um determinado mês.
  
## Resultados

 A partir do mês de Abril, o número acumulado de mortes por habitante apresentou uma correlação espacial no índice de Moran global (IM>0,50; P-valor<0,05) que se manteve em um alto patamar de correlação até o mês de Julho. Referente ao índice de Moran calculado para o número mensal de mortes por habitante (ou seja sem o efeito da sazionalidade), no mês de Abril o índice de Moran global (IM=0,72; P-valor<0,05) apresentou um alto patamar de dependênciav **(Tabela - 1)**, apresentando uma leve queda nos meses subsequentes em relação a Abril.
