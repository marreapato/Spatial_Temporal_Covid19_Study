...
## Introdução {.tabset .tabset-fade .tabset-pills}
 ...
 O objetivo deste trabalho será portanto traçar um perfil sobre o comportamento espacial da pandemia causada pela Covid-19 ao utilizar de técnicas estatísticas para quantificação da dependência espacial como o cálculo do índice de Moran global e local $\!\!^4$, bem como analisar as políticas adotadas por cada país e o comportamento da pandemia ao longo do tempo.
...
A partir do mês de Maio houve uma abrupta aceleração no crescimento da quantidade de casos acumulados por habitante em todos os continentes **(Figura - 17)**, sendo este mês o pico de contágio no primeiro semestre, se mantendo em um alto patamar até o mês de Julho. No entanto o crescimento do número de casos acumulados já é visível no mês de Abril em diversos continentes como na América do Norte, América do Sul, Ásia e Europa continente que apresentou dependência espacial no número cumulativo e mensal de mortes por habitantes já no mês de Abril apresentava uma dependência espacial local.
 ...
 Dentre as medidas restritivas adotadas **(Tabela 1)**, tanto as medidas de fechamento total de fronteiras como todas as outras políticas restritivas, atingiram seu pico no mês de Abril em todo o mundo, em que houve uma maior quantidade de países adotando essas políticas. Traçando um paralelo com a média móvel semanal do número de casos **(Figura - 18)**, houve um período de maior estabilidade na estatística em quase todo o mês, voltando a crescer aceleradamente no mês de Junho. Já em Abril, a média móvel do número de mortes **(Figura - 19)** que crescia de maneira bastante acelerada, apresentou uma leve desaceleração no mundo, porém voltou a crescer no mesmo mês e atingiu seu pico, caindo aceleradamente após o evento neste mês e voltando a crescer a partir de Junho.
 
 O Índice de Moran local sobre o número mensal de mortes por habitantes demonstrou uma forte dependência espacial na Europa de Abril até Maio, no mês de Junho e em seu subsequente a dependência local já não estava mais presente **(Figura - 20)**, enquanto o Índice sobre a quantidade acumulada **(Figura - 21)**, permaneceu por mais tempo. Em contrapartida a média móvel da quantidade de novos casos começou a cair a partir do mês de Abril no continente europeu, se mantendo em uma leve queda até o mês de Agosto. 

 Na América do Sul a média móvel de novos casos começou a subir a partir de Abril e nos meses subsequentes a dependência espacial referente ao número de mortes por habitantes é identificada em boa parte do continente. Nas Américas do Norte e Central a análise do comportamento dentre a média móvel dos novos casos e o índice de Moran local fica dificultada por apenas ter sido identificado dependência espacial local na América do Norte, porém o período de estabilidade dentre Maio e Junho é refletido como a dependência espacial entre o Canadá e os Estados Unidos.

 
## Conclusão

 Portanto, há uma dependência espacial no número de mortes por habitantes infectados pela Covid-19, que teve seu pico no primeiro semestre em Maio de 2020 e começou a registrar alto crescimento em Abril, apesar das similaridades políticas referente a poítica de fechamento internacional, esta dependência mais verificada apenas nos países da parte ocidental da Europa, na América do Sul e na América do Norte.
 
 Apesar dos países em que foi identificada correlação espacial apresentarem diferenças entre as políticas de restrição adotadas, em contraposição dos que não houve nenhum tipo de correlação, existe portanto a necessidade de se investigar o motivo de tal dependência espacial para o número de mortes por habitantes e o porquê dela estar localizada nestes continentes.
...
[8] - https://periodicos.utfpr.edu.br/rbgeo/article/view/5912/4915

[9] - Max Roser, Hannah Ritchie, Esteban Ortiz-Ospina and Joe Hasell (2020) - "Coronavirus Pandemic (COVID-19)". Published online at OurWorldInData.org. Retrieved from: 'https://ourworldindata.org/coronavirus' [Online Resource]

[10] - https://www.worldometers.info/geography/how-many-countries-are-there-in-the-world/
...
![](workplace.png)
**Figura 9 -** Políticas de fechamento de comèrcio adotadas.
...
![](media_movel.png)
**Figura 18 -** Média móvel semanal do número de novos casos da Covid-19.

![](n_mortes.png)
**Figura 19 -** Média móvel semanal do número de novas mortes pela Covid-19.

![](cum_local_death.png)
**Figura 20 -** Índice local de Moran do número acumulado de mortes por habitantes.

![](men_local_deaths.png)
**Figura 21 -** Índice local de Moran do número mensal de mortes por habitantes.

![](pol_meas.png)
**Figura 22 -** Proporção de países que adotaram medidas restritivas.
