...
#### A estatística de Scan Poisson
 ...
 Em busca de identificar *clusters* emergentes no espaço-tempo, o método prospectivo proposto por Kulldorff foi utilizado supondo uma distribuição de probabilidade poisson. Em contrapartida a estatística de scan puramente espacial $\!\!^1$ $\!\!^1$, em que são criadas várias janelas circulares em diversas localidades no mapa, cada uma com diferentes grupos de áreas vizinhas, sendo cada uma delas flexíveis em suas duas dimensões, tamanho e localidade, podendo ou não conter um *cluster* de eventos, a estatística de scan espaço-temporal cria janelas cilíndricas em três dimensões, onde a altura do cilindro representa o tempo, os cilindros são flexíveis em suas bases circulares e em suas datas de início, portanto para cada janela circular uma possível data para o início do *cluster* é considerada, em que apenas os *clusters* ativos, ou seja, os cilindros que chegarem ao final do período de estudo são considerados.
 
 segundo Kulldorff (citação colocar traduzida kulldorff) "The method strives to detect only those clusters that are still alive', meaning that the excess risk is still present during the last time period for which data are available. It can detect clusters irrespective of the cluster's `age', including long-existing clusters with moderately excess risk as well as recently emerged clusters with a high excess risk."(continuar escrevendo...)
