...
## Introdução {.tabset .tabset-fade .tabset-pills}
 ...
 At the end of 2019, the situation of infections by Covid-19 *(Sars-Cov-2)* was already a cause for concern, being considered as an epidemic in the city of Wuhan in China $\!\!^1$, although it was difficult at first to identify the virus' origin, it's known that the Severe Acute Respiratory Syndrome *(SARS)* and the Middle East Respiratory Syndrome *(MERS)* usually need intermediate hosts $\!\!^2$, there is a possibility that bats and pangolins were the first carriers. After several public organizations called for caution relating to Covid-19, on March 12 of 2020 the pandemic state was recognized by the World Health Organization *(WHO)* $\!\!^3$.
 
 With the whole world being affected by the pandemic, it's a matter of great importance that studies about the Spatio-temporal aspects of the Covid-19 pandemic should be conducted, by evaluating the number of cases and deaths in different countries accordingly to time in the year 2020, we can contribute with methods to analyze Spatio-temporal behavior in future disease outbreaks.
 
 The main goal of this paper is to present methods used to identify spatial and spatio-temporal dependence by using the Global Moran's I and the Local Moran's I, which is also known as a LISA statistic (Local Indicator of Spatial Association), both being used as indicators of spatial association.
 
 O objetivo deste trabalho será portanto traçar um perfil sobre o comportamento espacial da pandemia causada pela Covid-19 ao utilizar de técnicas estatísticas para quantificação da dependência espacial como o cálculo do índice de Moran global e local $\!\!^4$, além de uma estatística de scan para identificação de *clusters* emergentes do número de casos no espaço-tempo, bem como analisar as políticas adotadas por cada país e o comportamento da pandemia ao longo do tempo.
