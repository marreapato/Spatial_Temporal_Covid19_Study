...
#### Moran's I
 ...
 To evaluate the spatial association during the pandemic, the knn method was used to create spatial polygons and to build the spatial weight matrices, necessary to calculate the Global Moran's I and the *LISA*. The Global Moran's I is widely used and gathers all spatial dependence in one value $\!\!^6$, while the use of the Local Moran's I is important to identify patterns of spatial association, hence the *LISA* should be use with the Global Moran's I, as it's stated by Maia, Gomes and Almeida (2019, p.930) "while the global Moran's I may suggest, in general, that there is little spatial autocorrelation in the data, LISA values can identify smaller geographic areas where positive or negative clustering occurs.".
 
 , se faz útil pois de acordo com Maia, Gomes e Almeida (2018, p.930) enquanto o índice de Moran global pode sugerir que haja baixa correlação espacial nos dados, os valores do *LISA* podem identificar sub-regiões geográficas em que o agrupamento (*clustering*) negativo ou positivo ocorram $\!\!^7$.
...
A partir do mês de Maio houve uma abrupta aceleração no crescimento da quantidade de casos acumulados por habitante em todos os continentes **(Figura - 17)**, sendo este mês o pico de contágio no primeiro semestre, se mantendo em um alto patamar até o mês de Julho. No entanto o crescimento do número de casos acumulados já é visível no mês de Abril em diversos continentes como na América do Norte, América do Sul, Ásia e Europa continente que apresentou dependência espacial no número cumulativo e mensal de mortes por habitantes já no mês de Abril apresentava uma dependência espacial local.
 ...
 O Índice de Moran local sobre o número mensal de mortes por habitantes demonstrou uma forte dependência espacial na Europa de Abril até Maio, no mês de Junho e em seu subsequente a dependência local já não estava mais presente **(Figura - 20)**, enquanto o Índice sobre a quantidade acumulada **(Figura - 21)**, permaneceu por mais tempo. Em contrapartida a média móvel da quantidade de novos casos começou a cair a partir do mês de Abril no continente europeu, se mantendo em uma leve queda até o mês de Agosto. A dependência local se deu em todos os países com alto número de mortes por habitantes próximos a territórios que também tiveram alto número de mortes dentre seus cidadãos (AA e AA, ou AA e AB) **(Figura - 23)**.
